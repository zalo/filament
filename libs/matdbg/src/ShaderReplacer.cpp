...
void BlobIndex::writeChunks(ostream& stream) {
    ...
    vector<SpirvEntry> entries;
    for (const auto& record : mShaderRecords) {
        entries.push_back((SpirvEntry){
            .shaderModel = record.model,
            .variantKey = record.variant.key,
            .stage = record.stage,
            .dictionaryIndex = record.blobIndex
        });
    }
    ...
    filamat::ChunkContainer cc;
    cc.addChild<MaterialSpirvChunk>(std::move(entries));
