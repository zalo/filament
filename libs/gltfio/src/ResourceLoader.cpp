...
Texture* ResourceLoader::Impl::getOrCreateTexture(FFilamentAsset* asset, const TextureSlot& tb) {
    ...
    // Note that this is a data URI used directly in the image URI, which is distinct
    // from a data URI used as a buffer URI. The latter has already been decoded by
    // cgltf_load_buffers.
